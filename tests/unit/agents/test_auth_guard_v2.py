"""Unit tests for AuthGuardAgentV2 (Authentication Guard Agent v2).
...
from amoskys.agents.auth.auth_guard_agent_v2 import (
    AuthGuardAgentV2,
    AuthLogCollector,
    EventBusPublisher,
...
@pytest.fixture
def auth_agent():
    """Create AuthGuardAgentV2 with mocked dependencies."""
    with patch("amoskys.agents.auth.auth_guard_agent.EventBusPublisher"):
        with patch("amoskys.agents.auth.auth_guard_agent.LocalQueueAdapter"):
            with patch(
                ...
                agent = AuthGuardAgentV2(collection_interval=5.0)
                yield agent
...
class TestAuthGuardAgentV2Init:
    """Test agent initialization."""
