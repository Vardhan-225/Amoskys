"""Tests for FIMAgentV2 and associated components.

Covers:
    - FIMAgentV2 initialization and probe setup
...
from amoskys.agents.fim.fim_agent import BaselineEngine, FIMAgentV2
...
class TestFIMAgentV2Init:
    """Test FIMAgentV2 initialization."""
...
class TestFIMAgentV2Setup:
    """Test FIMAgentV2 setup and initialization."""
...
class TestFIMAgentV2Collection:
...
class TestFIMAgentV2Validation:
...
# ---------------------------------------------------------------------------
# NEW: FIMAgentV2 Setup Extended Tests
# ---------------------------------------------------------------------------


class TestFIMAgentV2SetupExtended:
    """Test FIMAgentV2 setup edge cases."""
    ...
    def test_setup_create_mode(self, tmp_path):
        ...
        with patch("amoskys.agents.fim.fim_agent.EventBusPublisher"):
            with patch("amoskys.agents.fim.fim_agent.LocalQueueAdapter"):
                agent = FIMAgentV2(
                    baseline_mode="create", monitor_paths=[str(tmp_path)]
                )
                result = agent.setup()
    ...
    def test_setup_monitor_no_baseline(self, tmp_path):
        ...
        with patch("amoskys.agents.fim.fim_agent.EventBusPublisher"):
            with patch("amoskys.agents.fim.fim_agent.LocalQueueAdapter"):
                agent = FIMAgentV2(
                    baseline_mode="monitor", monitor_paths=[str(tmp_path)]
                )
...
# ---------------------------------------------------------------------------
# NEW: FIMAgentV2 collect_data Extended Tests
# ---------------------------------------------------------------------------


class TestFIMAgentV2CollectExtended:
...
# ---------------------------------------------------------------------------
# NEW: FIMAgentV2 validate_event Extended Tests
# ---------------------------------------------------------------------------


class TestFIMAgentV2ValidateExtended:
...
# ---------------------------------------------------------------------------
# NEW: FIMAgentV2 Shutdown Tests
# ---------------------------------------------------------------------------


class TestFIMAgentV2Shutdown:
    """Test FIMAgentV2 shutdown behavior."""
...
# ---------------------------------------------------------------------------
# NEW: FIMAgentV2 get_health Tests
# ---------------------------------------------------------------------------


class TestFIMAgentV2GetHealth:
...
class TestFIMPlatformPaths:
    ...
    def test_custom_monitor_paths(self, tmp_path):
        """Test FIMAgentV2 with custom monitor paths."""
        with patch("amoskys.agents.fim.fim_agent.EventBusPublisher"):
            with patch("amoskys.agents.fim.fim_agent.LocalQueueAdapter"):
                custom = [str(tmp_path)]
                agent = FIMAgentV2(monitor_paths=custom)
