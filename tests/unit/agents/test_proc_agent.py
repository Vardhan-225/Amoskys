"""Unit tests for ProcAgent (Process Agent v3) with Micro-Probe Architecture.
...
pytest.skip(
    "macOS Observatory v2 renamed ProcAgent to MacOSProcessAgent; removed EventBusPublisher, SystemMetricsCollector; probe names changed (no PROC_PROBES, HighCPUAndMemoryProbe, LOLBinExecutionProbe, LongLivedProcessProbe, ProcessInfo, ProcessTreeAnomalyProbe, SuspiciousUserProcessProbe, _make_process_guid)",
...
from amoskys.agents.os.macos.process.agent import (
    EventBusPublisher,
    ProcAgent,
    SystemMetricsCollector,
)
...
def proc_agent(mock_eventbus, mock_queue_adapter):
    ...
    with patch(
        "amoskys.agents.os.macos.process.agent.EventBusPublisher",
        return_value=mock_eventbus,
    ):
        with patch(
            "amoskys.agents.os.macos.process.agent.LocalQueueAdapter",
            return_value=mock_queue_adapter,
        ):
            with patch(
                "amoskys.agents.os.macos.process.agent.create_proc_probes",
                return_value=[],
            ):
...
class TestValidationExtended:
    """Extended validation tests for proc_agent validate_event."""
...
class TestCollectDataProtobuf:
    ...
    def test_collect_data_returns_device_telemetry(self, proc_agent, stub_probe):
        ...
        if result:
            ...
            assert result[0].collection_agent == "proc"
...
class TestProcessSpawnProbe:
    ...
    @patch("amoskys.agents.os.macos.process.probes.PSUTIL_AVAILABLE", False)
    ...
    @patch("amoskys.agents.os.macos.process.probes.psutil")
...
class TestDylibInjectionProbeUnit:
    ...
    @patch("amoskys.agents.os.macos.process.probes.platform")
    ...
    def test_detects_dyld_injection(self, mock_run, mock_platform):
        ...
        probe = DylibInjectionProbe()
        with patch("amoskys.agents.os.macos.process.probes.psutil") as mock_psutil:
            mock_psutil.NoSuchProcess = psutil.NoSuchProcess
            mock_psutil.AccessDenied = psutil.AccessDenied
            proc_mock = MagicMock()
