"""Tests for DeviceDiscovery network asset discovery.

Covers:
    - DeviceDiscovery initialization and probe setup
...
def device_discovery_agent():
    """Create DeviceDiscovery instance for testing."""
    return DeviceDiscovery(device_id="test-host-001")


@pytest.fixture
def device_discovery_agent_with_config():
    """Create DeviceDiscovery with known devices configured."""
    known_ips = {
        "192.168.1.1",  # Gateway
...
def device_discovery_agent_with_mocks(tmp_path):
    """Create DeviceDiscovery with mocked queue."""
    mock_queue = MagicMock()
    agent = DeviceDiscovery(
        device_id="test-host-001",
...
class TestDeviceDiscoveryInit:
    """Test DeviceDiscovery initialization."""
    ...
    def test_custom_collection_interval(self):
        ...
        agent = DeviceDiscovery(device_id="test-host", collection_interval=20.0)
        assert agent.collection_interval == 20.0


class TestDeviceDiscoverySetup:
    """Test DeviceDiscovery setup and initialization."""
...
class TestDeviceDiscoveryCollection:
...
class TestDeviceDiscoveryKnownDeviceManagement:
...
class TestDeviceDiscoveryHealth:
...
class TestDeviceDiscoveryValidation:
...
class TestDeviceDiscoverySharedData:
