...
class ProcessMonitor:
    ...
    def detect_changes(
        ...
        terminated_processes = [
            self.processes[pid] for pid in terminated_pids if pid in self.processes
        ]
...
class ProcAgent:
    ...
    def __init__(
        self, collection_interval: int = 30, suspicious_patterns: List[str] = None
    ):
        ...
        try:
            ...
            result = {
                ...
                "top_cpu": [
                    {
                        ...
                        "name": p.name,
                        "cpu_percent": (
                            p.cpu_percent if p.cpu_percent is not None else 0.0
                        ),
                    }
                    for p in top_cpu[:5]
                ],
                "top_memory": [
                    {
                        ...
                        "name": p.name,
                        "memory_percent": (
                            p.memory_percent if p.memory_percent is not None else 0.0
                        ),
                    }
                ...
                "suspicious": [
                    ...
                    for p in changes["suspicious"]
                ],
            }

            logger.info(
                f"✅ Collected: {len(processes)} processes, {len(changes['new'])} new, {len(changes['suspicious'])} suspicious"
            )
...
async def main():
    ...
    for i, proc in enumerate(data.get("top_cpu", []), 1):
        print(
            f"  {i}. {proc['name']:<20} PID {proc['pid']:<8} CPU {proc['cpu_percent']:.1f}%"
        )

    print(f"\n💾 Top 5 Memory consumers:")
    for i, proc in enumerate(data.get("top_memory", []), 1):
        print(
            f"  {i}. {proc['name']:<20} PID {proc['pid']:<8} MEM {proc['memory_percent']:.1f}%"
        )
