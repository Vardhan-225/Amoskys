...
@pytest.mark.skip(
    reason="flowagent/main.py does not exist — test references unimplemented module"
)
def test_wal_grows_then_drains():
    ...
    agent = subprocess.Popen(
        [sys.executable, "src/amoskys/agents/flowagent/main.py"], env=env
    )
    ...
    body = (
        urllib.request.urlopen("http://localhost:8081/healthz", timeout=2)
        .read()
        .decode()
    )
