...
from amoskys.agents.Web.argos.recon.stealth import StealthDossier, StealthFinding
from amoskys.agents.Web.argos.stage1 import PitchDossier, PitchFinding, Stage1
...
def test_stage1_runs_with_no_findings_cleanly():
    with (
        patch(
            "amoskys.agents.Web.argos.stage1.StealthRecon.run",
...
def test_stage1_translates_dev_leaks_to_pitch_finding():
    findings = [
        _mk_sf(
            ...
            "high",
            title="/.git exposed",
            observed="/.git/config 200",
        ),
        _mk_sf(
            "dev_leaks", "dev.env", "high", title="/.env exposed", observed="/.env 200"
        ),
    ]
    with (
        patch(
...
def test_stage1_translates_user_enum_as_high():
    findings = [
        _mk_sf(
            ...
            "users.wp_rest",
            "high",
            title="3 users disclosed",
            observed="admin, editor, author",
        ),
    ]
    with (
        patch(
...
def test_email_note_always_includes_public_only_disclosure():
    ...
    assert "not probed" in text.lower() or "without authorization" in text.lower()
