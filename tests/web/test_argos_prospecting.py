...
from amoskys.agents.Web.argos.prospecting.ct_discovery import _registered_domain
...
def test_wp_indicator_flags_bug_bounty():
    body = (
        "<html><body>See our bug bounty program on "
    ...
    http = _fake_http(
        {
            "/": (
                200,
                {"cf-ray": "abc-DFW", "server": "cloudflare"},
                '<html><meta name="generator" content="WordPress 6.4"/></html>',
            ),
        }
    )
...
def test_score_bug_bounty_kills_score():
    ind = WPIndicatorResult(
        ...
        contact_hints=["mailto:sec@acme.com"],
        on_bug_bounty=True,
        bounty_evidence="via security.txt Policy",
    )
    p = score_prospect(ind)
    ...
    http_indicator = _fake_http(
        {
            "/": (
                200,
                {},
                '<html><meta name="generator" content="WordPress 6.9"/></html>',
            ),
        ...
        if host == "plain.com":
            ...
            routes = {
                "/": (
                    200,
                    {},
                    """<html><meta name="generator" content="WordPress 5.1"/>
