...
def _f(kind, loc="/", sev="medium", ev="", **meta):
    return ChainFinding(
        kind=kind, location=loc, severity=sev, evidence=ev, metadata=meta
    )
...
def test_smuggling_plus_waf_fires_when_waf_present():
    ...
    rep = reason_chains(
        [_f("smuggling", "/", "high", "CL.TE timing anomaly 3s")], profile=prof
    )
...
def test_smuggling_without_waf_does_not_fire_that_rule():
    ...
    assert all(
        "smuggling" not in c.name.lower() and "WAF" not in c.name for c in rep.chains
    )
...
def test_cdn_bypass_plus_verbose_origin_fires():
    ...
    rep = reason_chains(
        [_f("cdn_bypass", "203.0.113.1", "high", "direct IP reached")], profile=prof
    )
...
def test_narrative_contains_concrete_steps():
    rep = reason_chains(
        [_f("rest_authz", "/wp-json/plug/update", "high", "perm_cb missing")]
    )
    ch = rep.chains[0]
