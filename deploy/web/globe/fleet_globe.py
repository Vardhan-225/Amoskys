...
SEED_FLEET: List[Dict[str, Any]] = [
    ...
    {
        "domain": "donate.nonprofit-foundation.org",
        "country": "IE",
    ...
    {
        "domain": "greenfield-clinic.health",
        "country": "CA",
        "country_name": "Canada",
        "lat": 43.65,
        "lng": -79.38,
    ...
    {
        "domain": "mediterranean-eats.com.au",
        "country": "AU",
...
SCANNER_ORIGINS: List[Dict[str, Any]] = [
    ...
    {
        "label": "104.252.191.228",
        "asn_org": "ShadowServer",
        "country": "US",
        "lat": 39.04,
        "lng": -77.49,
    },
    {
        "label": "185.224.130.72",
        "asn_org": "Linode",
        "country": "DE",
        "lat": 50.11,
        "lng": 8.68,
...
def build_sites_view() -> List[Dict[str, Any]]:
    ...
    for s in SEED_FLEET:
        attacks = (
            rng.randint(0, 12)
            if s["posture"] == "healthy"
            else rng.randint(4, 38) if s["posture"] == "warn" else rng.randint(20, 120)
        )
...
def build_arcs_json(sites: List[Dict[str, Any]]) -> str:
    ...
    for site in sites:
        ...
        n_arcs = (
            1 if site["posture"] == "healthy" else 2 if site["posture"] == "warn" else 3
        )
...
def build_stats(
    sites: List[Dict[str, Any]], arcs_count: int, aegis_chain_pct: float = 92.2
) -> Dict[str, Any]:
    ...
    events_24h = (
        attacks_24h * 12
    )  # approximation: more events than attacks due to http/db volume
    return {
        "sites": len(sites),
        "countries": countries,
