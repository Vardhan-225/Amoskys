...
def igris_fleet_posture() -> dict:
    ...
    # Fleet health
    offline_pct = (
        ((total_devices - online) / total_devices * 100) if total_devices else 0
    )
...
def igris_device_risk(device_id: str) -> dict:
    ...
    malicious = sum(
        1 for r in classifications if r.get("final_classification") == "malicious"
    )
    suspicious = sum(
        1 for r in classifications if r.get("final_classification") == "suspicious"
    )
