...
from .db import execute, hours_ago_epoch, hours_ago_ns, query, scalar, write_conn
...
def _incident_category(signal_type: str) -> str:
    return (
        "operational" if signal_type.startswith(_OPERATIONAL_PREFIXES) else "security"
    )
...
class IGRISCloudBrain:
    ...
    def status(self) -> dict:
        return {
            ...
            "uptime_hours": (
                round((time.time() - self._state.started_at) / 3600, 2)
    ...
    def _observe_cycle(self) -> None:
        ...
        if signals or actions:
            logger.info(
                ...
                cycle,
                self._state.posture,
                self._state.threat_level,
                len(signals),
                len(actions),
                self._state.last_cycle_duration_ms,
            )
    ...
    def _collect_metrics(self) -> dict[str, Any]:
        ...
        m["fleet.online"] = (
            scalar("SELECT COUNT(*) FROM devices WHERE last_seen > ?", (now - 120,))
            or 0
        )
        ...
        m["events.freshness_s"] = (now - latest_ns / 1e9) if latest_ns else 9999
    ...
    def _check_hard_threshold(
        self, key: str, value: Any, now: float
    ...
    def _update_posture(self, metrics: dict, signals: list[BrainSignal]) -> None:
        ...
        if critical_signals > 0 or critical_incidents > 0 or critical_events >= 5:
            ...
            self._state.threat_level = max(
                0.0, min(0.24, metrics.get("events.1h", 0) * 0.001)
            )
    ...
    def _decide_and_act(self, signals: list[BrainSignal]) -> list[dict]:
        ...
        for signal in signals:
            ...
            if signal.confidence >= GATE_ALERT and signal.severity in (
                "high",
                "critical",
            ):
                try:
                    ...
                    logger.exception(
                        "Failed to upsert incident for signal %s", signal.signal_id
                    )

            # Queue containment for very high confidence lateral movement
            if (
                signal.confidence >= GATE_CONTAIN
                        ...
                        for dev_id in signal.device_ids:
                            try:
                                ...
                                _queue_command(
                                    ...
                                    {
                                        "ip": ip,
                                        "reason": f"IGRIS Brain: {signal.message}",
                                        "duration_s": 3600,
                                    },
                                    priority=1,
                                    ttl=3600,
                                ...
                                logger.exception(
                                    "Failed to block IP %s on %s", ip, dev_id
                                )
    ...
    def _self_heal_cycle(self, metrics: dict[str, Any]) -> None:
        ...
        if freshness > 600:  # >10 min without data
            ...
            logger.warning(
                "Brain self-heal: triggered collection on %d devices (freshness=%ds)",
                len(devices),
                freshness,
            )
        ...
        for dev in devices:
            ...
            for agent_name in missing:
                try:
                    ...
                    _queue_command(
                        did, "RESTART_AGENT", {"agent_name": agent_name}, priority=3
                    )
                    healed.append(
                        {
            ...
            if missing:
                logger.info(
                    "Brain self-heal: restarting %d silent agents on %s",
                    len(missing),
                    dev.get("hostname", did[:8]),
                )
    ...
    def _upsert_incident(self, signal: "BrainSignal", now: float) -> str:
        ...
        execute(
            ...
            (
                ...
                now,
                key,
                _incident_category(signal.signal_type),
                now,
            ),
        )
        return "created"
