...
class AmoskysConfig:
    ...
    def from_env(cls) -> "AmoskysConfig":
        ...
        config.eventbus.overload_mode = os.getenv("BUS_OVERLOAD", "false").lower() in (
            "1",
            "true",
            "on",
            "yes",
        )
        config.eventbus.max_inflight = int(
            os.getenv("BUS_MAX_INFLIGHT", str(config.eventbus.max_inflight))
        )
        config.eventbus.hard_max = int(
            os.getenv("BUS_HARD_MAX", str(config.eventbus.hard_max))
        )
        config.eventbus.metrics_port_1 = int(
            os.getenv("BUS_METRICS_PORT_1", str(config.eventbus.metrics_port_1))
        )
        config.eventbus.metrics_port_2 = int(
            os.getenv("BUS_METRICS_PORT_2", str(config.eventbus.metrics_port_2))
        )
        config.eventbus.health_port = int(
            os.getenv("BUS_HEALTH_PORT", str(config.eventbus.health_port))
        )
        config.eventbus.metrics_disabled = os.getenv("BUS_METRICS_DISABLE", "") in (
            "1",
            "true",
            "on",
            "yes",
        )
        config.eventbus.log_level = os.getenv(
            "LOGLEVEL", config.eventbus.log_level
        ).upper()
        ...
        config.agent.max_env_bytes = int(
            os.getenv("IS_MAX_ENV_BYTES", str(config.agent.max_env_bytes))
        )
        config.agent.send_rate = int(
            os.getenv("IS_SEND_RATE", str(config.agent.send_rate))
        )
        config.agent.retry_max = int(
            os.getenv("IS_RETRY_MAX", str(config.agent.retry_max))
        )
        config.agent.retry_timeout = float(
            os.getenv("IS_RETRY_TIMEOUT", str(config.agent.retry_timeout))
        )
        config.agent.metrics_port = int(
            os.getenv("IS_METRICS_PORT", str(config.agent.metrics_port))
        )
        config.agent.health_port = int(
            os.getenv("IS_HEALTH_PORT", str(config.agent.health_port))
        )
        ...
        config.storage.storage_dir = os.getenv(
            "IS_STORAGE_DIR", config.storage.storage_dir
        )
        config.storage.max_wal_bytes = int(
            os.getenv("IS_MAX_WAL_BYTES", str(config.storage.max_wal_bytes))
        )
