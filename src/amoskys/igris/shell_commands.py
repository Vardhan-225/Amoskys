...
def show_igris_briefing():
    ...
    print()
    print(
        f"  Posture:  {pc}{posture}{C.RESET} (threat: {_risk_bar(threat)}) {C.DIM}{ti} {trend}{C.RESET}"
    )
    ...
    if directives:
        ...
        for d in directives[:8]:
            ...
            print(
                f"    {uc}{urgency:8s}{C.RESET} {dtype} {target}{stage_str}{novel_str}"
            )
...
def show_igris_chain():
    ...
    for i, stage in enumerate(stages):
        ...
        next_indicator = (
            f" {C.YELLOW}\u2190 PREDICTED NEXT{C.RESET}" if stage == next_stage else ""
        )
...
def show_igris_why(target: str = ""):
    ...
    if target:
        directives = [d for d in directives if target in str(d.get("target", ""))]
        if not directives:
...
def show_igris_inspect(args: str):
    ...
    if len(parts) < 2:
        ...
        print(
            f"  {C.DIM}Actions: codesign, connections, children, xattr, plist, hash, lsof, environ{C.RESET}"
        )
        return
...
def show_igris_novel():
    ...
    try:
        ...
        print()
        print(
            f"  {C.BOLD}{C.MAGENTA}SOMA: Novel Patterns{C.RESET} ({len(novel)} in last hour)"
        )
        print()
        for n in novel:
...
def handle_igris_command(args: str) -> bool:
    ...
    if not subcmd or subcmd == "briefing":
        ...
        print(
            f"  {C.DIM}Available: chain, why, inspect, memory, novel, history, stats{C.RESET}"
        )
