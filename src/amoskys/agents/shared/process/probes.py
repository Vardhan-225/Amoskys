...
class SuspiciousUserProcessProbe(MicroProbe):
    ...
    # NOTE: mysqld and postgres intentionally excluded — they are designed to
    # run as non-root (Homebrew installs as current user, production runs as
    # dedicated service accounts like mysql/postgres). Flagging them produces
    # false positives on every macOS/Linux dev machine.
    ROOT_ONLY_PROCESSES = {
        "sshd",
        "httpd",
        "nginx",
...
class BinaryFromTempProbe(MicroProbe):
    ...
    def _is_temp_path(self, path: str) -> bool:
        """Check if a path matches any temp directory pattern."""
    ...
    @staticmethod
    def _is_app_translocation(path: str) -> bool:
        """Check if path is macOS App Translocation (Gatekeeper security feature).

        macOS moves quarantined apps to /private/var/folders/*/T/AppTranslocation/
        before execution. This is the OS enforcing security, not malware.
        """
        return "AppTranslocation" in path

    def _find_temp_execution(self, info: dict) -> Optional[str]:
        """Return the matched temp path from exe or cmdline, or None."""
        exe = info.get("exe", "") or ""

        # Check exe path directly
        if exe and self._is_temp_path(exe) and not self._is_app_translocation(exe):
            return exe

        # Check cmdline for script-from-temp (BLIND-3 fix):
        # When a script runs from /tmp, exe shows the interpreter (e.g. /bin/bash)
        # but the actual script path is in cmdline[1] or cmdline[2].
        ...
        for arg in cmdline[1:3]:
            if (
                isinstance(arg, str)
                and self._is_temp_path(arg)
                and not self._is_app_translocation(arg)
            ):
                return arg
    ...
    def scan(self, context: ProbeContext) -> List[TelemetryEvent]:
        ...
        for proc in psutil.process_iter(
            ...
            try:
                ...
                matched_path = self._find_temp_execution(info)
...
class CodeSigningProbe(MicroProbe):
    ...
    # /usr/libexec/securityd removed: doesn't exist on macOS 26.0.
    # /usr/bin/python removed: macOS 26.0 ships python3 only.
    # Permission-denied errors (e.g. /usr/bin/sudo as non-root) are handled
    # gracefully — they produce a debug log, not a HIGH alert.
    CRITICAL_BINARIES = [
        "/usr/sbin/sshd",
        "/usr/bin/sudo",
        "/bin/bash",
    ...
    # Errors that indicate a permission boundary, NOT a tampered binary
    ...
    def scan(self, context: ProbeContext) -> List[TelemetryEvent]:
        ...
        for binary_path in self.CRITICAL_BINARIES:
            ...
            try:
                ...
                # Permission denied is NOT an invalid signature — it means
                # the binary requires root for verification (macOS SIP).
                if any(err in stderr for err in self._PERMISSION_ERRORS):
                    logger.debug(
                        "CodeSigningProbe: %s requires elevated privileges to verify",
                        binary_path,
                    )
                    continue
                ...
                events.append(
                    self._create_event(
                        ...
                        data={
                            ...
                            "codesign_error": stderr or "Unknown error",
