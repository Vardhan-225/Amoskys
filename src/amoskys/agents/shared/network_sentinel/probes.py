...
class HTTPScanStormProbe(MicroProbe):
    ...
    def scan(self, context: ProbeContext) -> List[TelemetryEvent]:
        ...
        for ip, paths in ip_paths.items():
            ...
            severity = Severity.CRITICAL if unique_count >= self.CRITICAL_THRESHOLD else Severity.HIGH
            ...
            logger.warning(
                ...
                ip, unique_count, total_reqs, rps,
            )
...
class SQLiPayloadProbe(MicroProbe):
    ...
    def scan(self, context: ProbeContext) -> List[TelemetryEvent]:
        ...
        for txn in transactions:
            ...
            severity = Severity.CRITICAL if len(matched_patterns) >= 3 else Severity.HIGH
...
_ADMIN_PATHS: Set[str] = {
    ...
    "/api/debug", "/api/config", "/api/admin",
    "/manage", "/management",
...
class ConnectionFloodProbe(MicroProbe):
    ...
    def scan(self, context: ProbeContext) -> List[TelemetryEvent]:
        ...
        for conn in connections:
            ...
            remote_ip = src_ip if dst_ip in ("127.0.0.1", "0.0.0.0", "::1", "*") else src_ip
