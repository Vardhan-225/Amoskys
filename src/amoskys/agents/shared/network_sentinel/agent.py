...
from amoskys.agents.shared.network_sentinel.collector import (
    AccessLogCollector,
    ConnectionStateCollector,
)
from amoskys.agents.shared.network_sentinel.probes import (
    ...
    def collect_data(self) -> Sequence[Any]:
        ...
        if events:
            logger.warning(
                ...
                len(events), len(http_transactions), len(connections),
            )
        else:
            logger.info(
