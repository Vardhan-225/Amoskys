...
from amoskys.agents.shared.db_activity.agent_types import (
    DB_PORTS,
    QUERY_TYPE_MAP,
    DatabaseQuery,
)
from amoskys.agents.shared.db_activity.probes import create_db_activity_probes
...
class MacOSDBCollector(DBCollector):
    ...
    def _collect_sqlite_wal(self) -> List[DatabaseQuery]:
        ...
        try:
            ...
            if not getattr(self, "_fs_usage_warned", False):
                logger.warning(
                    "fs_usage not available — SQLite WAL monitoring disabled"
                )
                self._fs_usage_warned = True
        except Exception as e:
