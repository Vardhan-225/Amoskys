...
from amoskys.agents.common.ip_utils import is_private_ip
...
from amoskys.agents.shared.network.probes import FlowEvent, create_flow_probes
...
class MacOSFlowCollector:
    ...
    def __init__(self, interface: Optional[str] = None):
        ...
        self._nettop = None
        self._flow_state = FlowStateTable()
    ...
    def collect(self, window_seconds: int = 60) -> List[FlowEvent]:
        ...
        # Collect nettop byte counts and enrich process names
        nettop_records = None
        if self._nettop and flows:
            try:
                ...
                logger.debug("nettop collection failed: %s", e)

        # Stateful tracking: merge with FlowStateTable for duration,
        # seen_count, and per-flow byte estimation
        if flows:
            self._flow_state.update(flows, nettop_records)
            logger.debug(
                "FlowStateTable tracking %d connections",
                self._flow_state.tracked_count,
            )
...
class FlowAgent(MicroProbeAgentMixin, HardenedAgentBase):
        ...
        try:
            ...
            for cert in required_certs:
                ...
                if not os.path.exists(cert_path):
                    logger.warning(
                        "Certificate not found: %s (EventBus will fail)", cert_path
                    )
            ...
            # Setup probes
            if not self.setup_probes(collector_shared_data_keys=["flows"]):
    ...
    def collect_data(self) -> Sequence[Any]:
        ...
        context.shared_data["window_start_ns"] = timestamp_ns - int(
            self.collection_interval * 1e9
        )
        ...
        # Emit flow telemetry for ALL external (public-IP) connections
        # so GeoIP/ASN enrichment populates flow_events with real data.
        seen_flows = set()
        flow_telem_count = 0
        for flow in flows:
            if not flow.dst_ip or is_private_ip(flow.dst_ip) or flow.dst_ip == "*":
                continue
            ...
            seen_flows.add(key)
            flow_telem_count += 1
            se = telemetry_pb2.SecurityEvent(
                event_category="network_flow",
                event_action=f"{flow.dst_ip}:{flow.dst_port}",
                risk_score=0.0,
                analyst_notes="Baseline flow telemetry",
            )
            te = telemetry_pb2.TelemetryEvent(
                event_id=f"flow_telem_{flow.dst_ip}_{flow.dst_port}_{timestamp_ns}",
                event_type="SECURITY",
                severity="DEBUG",
                event_timestamp_ns=timestamp_ns,
                source_component="flow_telemetry",
                security_event=se,
                confidence_score=0.0,
            )
            te.attributes["src_ip"] = flow.src_ip or ""
            te.attributes["dst_ip"] = flow.dst_ip
            te.attributes["src_port"] = str(flow.src_port)
            te.attributes["dst_port"] = str(flow.dst_port)
            te.attributes["protocol"] = flow.protocol
            te.attributes["bytes_tx"] = str(flow.bytes_tx)
            te.attributes["bytes_rx"] = str(flow.bytes_rx)
            proto_events.append(te)
            if flow_telem_count >= 50:
                break

        if flow_telem_count > 0:
            logger.info("Emitted %d flow telemetry events (external IPs)", flow_telem_count)
