...
class KernelAuditAgentV2(MicroProbeAgentMixin, HardenedAgentBase):
    ...
    def setup(self) -> bool:
        ...
        """
        logger.info(
            "Setting up %s for device %s", self.agent_name, self.device_id
        )
        ...
        if not self._probes:
            ...
            logger.info("Registered %d default kernel audit probes", len(default_probes))
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        base_health.update({
            "collector_type": type(self._collector).__name__ if self._collector else None,
