...
class DeviceDiscoveryEngine:
    ...
    def _load_device_signatures(self) -> Dict:
        ...
        return {
            ...
            "iot_devices": {
                "generic_mqtt": {"ports": [1883, 8883], "device_type": "IOT"},  # MQTT
                "generic_coap": {"ports": [5683, 5684], "device_type": "IOT"},  # CoAP
            },
            "network_equipment": {
                "cisco_devices": {
    ...
    async def _discover_network_range(self, network_range: NetworkRange):
        ...
        for ip in network.hosts():
            ...
            task = asyncio.create_task(
                self._discover_single_host(str(ip), network_range)
            )
            discovery_tasks.append(task)
    ...
    async def _async_scan_ports(
        self, ip_address: str, common_ports: Optional[List[int]] = None
    ) -> List[int]:
    ...
    async def _check_tcp_port(
        self, ip_address: str, port: int, timeout: float = 2.0
    ) -> bool:
    ...
    async def _identify_device(self, ip_address: str, open_ports: List[int]) -> Dict:
        """Identify device type and characteristics"""
        device_info = {"type": "UNKNOWN", "protocols": [], "metadata": {}}
    ...
    def _matches_signature(self, open_ports: List[int], signature: Dict) -> bool:
    ...
    def _determine_protocols(
        self, open_ports: List[int], device_info: Dict
    ) -> List[str]:
    ...
    async def _setup_telemetry_collection(self, device: DiscoveredDevice):
        ...
        try:
            ...
            if collection_config:
                ...
                logger.info(
                    f"Setting up {collection_config['protocol']} "
        ...
        for protocol in preferred_protocols:
            if protocol in protocols:
                return {
                    ...
                    "priority": (
                        "high" if device_type in ["MEDICAL", "INDUSTRIAL"] else "medium"
                    ),
                }
    ...
    def _scan_ports(
        self, ip_address: str, ports: List[int], timeout: float = 1.0
    ) -> List[int]:
    ...
    def _assess_vulnerability_risk(self, device_info: Dict) -> float:
        ...
        for port, service_info in services.items():
            ...
            if any(
                old_version in version
                for old_version in ["7.4", "2.4.6", "old", "legacy"]
            ):
                score += 0.1
        ...
        try:
            ...
            for ip in network.hosts():
                ...
                if open_ports:
                    ...
                    services = {
                        port: {"service": f"service_{port}"} for port in open_ports
                    }
                    device_type = self._fingerprint_device(str(ip), services)
                    device_info["device_type"] = device_type
                    device_info["services"] = services
                    device_info["risk_score"] = self._assess_vulnerability_risk(
                        device_info
                    )
