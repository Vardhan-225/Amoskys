...
class PeripheralAgent:
            ...
            def parse_usb_tree(items, devices_list):
                for item in items:
                    ...
                    if '_name' in item and item.get('_name') != 'USB':
                        ...
                        device['device_id'] = hashlib.md5(device_id.encode()).hexdigest()[:16]
                ...
                if line.startswith('Bus '):
                    ...
                    if len(parts) >= 6:
                        device = {
                            ...
                            'product_id': parts[5].split(':')[1] if ':' in parts[5] else '',
    ...
    def _create_telemetry(self, devices: List[Dict], events: Dict) -> telemetry_pb2.DeviceTelemetry:
        ...
        for device in events['connected']:
            ...
            event = telemetry_pb2.TelemetryEvent(
                ...
                attributes={
                    ...
                    'is_authorized': str(device['device_id'] in self.authorized_devices),
    ...
    def _publish_telemetry(self, device_telemetry: telemetry_pb2.DeviceTelemetry) -> bool:
        ...
        try:
            ...
            if ack.status == telemetry_pb2.UniversalAck.OK:
                logger.info(f"Published peripheral telemetry ({len(device_telemetry.events)} events)")
                return True
            else:
        ...
        try:
            ...
            if events['connected']:
                ...
                for dev in events['connected']:
                    logger.info(f"  → {dev['name']} ({self._classify_device_type(dev)})")
