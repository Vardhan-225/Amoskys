...
class AttackEdge:
    ...
    def activated_by(self, findings: List, profile=None) -> Optional[Any]:
        ...
        if not self.trigger_kinds:
            ...
            return None
        for f in findings:
            k = getattr(f, "kind", None) or (
                f.get("kind") if isinstance(f, dict) else None
            )
...
def _is_xss_related_cve(f, profile):
    ev = (getattr(f, "evidence", "") or "").lower()
    return any(
        kw in ev for kw in ("xss", "cross-site scripting", "stored xss", "script")
    )


def _is_user_enum_leak(f, profile):
    return (
        "user" in (getattr(f, "location", "") or "").lower()
        or "user" in (getattr(f, "evidence", "") or "").lower()
    )
...
def build_wordpress_graph() -> AttackGraph:
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "GET /wp-json/wp/v2/users?per_page=100 — public by "
                "default, returns every registered user's id + slug."
            ),
            defender_should_see=(
                "1+ hits on /wp-json/wp/v2/users from a new IP. "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "Run the public PoC for the matched CVE (see "
                "Patchstack / Wordfence writeup) against the "
                "unpatched plugin version."
            ),
            defender_should_see=(
                "Wordfence premium signature for that CVE "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "sqlmap against the affected parameter documented "
    ...
    g.add(
        AttackEdge(
            ...
            defense_pruned_by=("wordfence", "aegis"),
            attacker_action=(
                "Upload PHP polyglot (GIF89a;<?php…?>) via the "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "GET /wp-content/uploads/shell.php?cmd=id — PHP "
                "executes the embedded shell primitives."
            ),
            defender_should_see=(
                "Unusual 200 on a .php in /wp-content/uploads. "
    ...
    g.add(
        AttackEdge(
            ...
            mitre_technique="T1505.003 — Web Shell",
            attacker_action=(
                "Upload a crafted plugin ZIP via /wp-admin/plugin-"
                "install.php?action=upload-plugin. Activate it. "
                "Plugin runs arbitrary PHP on activation."
            ),
            defender_should_see=(
                "plugin_install log from unexpected IP; any "
    ...
    g.add(
        AttackEdge(
            ...
            mitre_technique="T1078 — Valid Accounts",
            attacker_action=(
                "POST /wp-login.php with recovered credentials. "
                "If the cracked account is admin, immediate access."
            ),
            defender_should_see=(
                "login success from new geo/IP. MFA or " "geo-fencing would block this."
            ),
        )
    )
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "Shell reads wp-config.php — extracts DB_USER, "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "Drop a must-use-plugin at wp-content/mu-plugins/ "
                "(auto-loaded, survives core updates) + wp_cron "
                "entry pinging a C2 every 15 min."
            ),
            defender_should_see=(
                "new file in /wp-content/mu-plugins/. Aegis "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "INSERT INTO wp_users (user_login,user_pass,...) + "
    ...
    g.add(
        AttackEdge(
            ...
            mitre_technique="T1059 — Command and Scripting Interpreter",
            attacker_action=(
                "Use theme-editor or plugin-editor in wp-admin to "
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "mysqldump wp_users + WooCommerce tables + any "
                "PII-bearing plugins. Compress and POST to attacker "
                "C2 or tunnel via DNS-over-HTTPS."
            ),
            defender_should_see=(
                "egress traffic spike; outbound DNS TXT " "queries of anomalous size."
            ),
        )
    )
    ...
    g.add(
        AttackEdge(
            ...
            attacker_action=(
                "If on shared hosting: scan ../ for other WP sites "
    ...
    g.add(
        AttackEdge(
            ...
            defense_pruned_by=("wordfence",),
            attacker_action=(
                "POST /wp-json/<ns>/<route> anonymously (no "
    ...
    g.add(
        AttackEdge(
            ...
            defense_pruned_by=("wordfence", "cloudflare"),
            attacker_action=(
                "LFI reads /var/www/html/wp-config.php via "
    ...
    g.add(
        AttackEdge(
            ...
            mitre_technique="",
            attacker_action=(
                "Once persistence is established, attacker returns "
