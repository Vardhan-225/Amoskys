...
_CVE_CATALOG: List[WpCve] = [
    # WP core
    WpCve("CVE-2024-4439",  "wordpress-core", "<= 6.5.1",
          "Stored XSS in Avatar Block (auth required)", 6.5),
    WpCve("CVE-2023-2745",  "wordpress-core", "<= 6.2.0",
          "Directory traversal via translation files", 5.4),
    # Top plugin CVEs (2023–2024, all public, mostly unauth)
    WpCve("CVE-2024-10924", "plugin:really-simple-security", "<= 9.1.1",
          "Authentication bypass via 2FA check flaw", 9.8),
    WpCve("CVE-2024-25600", "plugin:bricks", "<= 1.9.6",
          "Unauthenticated RCE via eval()", 9.8),
    WpCve("CVE-2024-6297",  "plugin:forminator", "<= 1.29.0",
          "Unauthenticated arbitrary file upload", 9.8),
    WpCve("CVE-2024-1207",  "plugin:litespeed-cache", "<= 5.7",
          "Unauth account takeover via weak hash", 8.1),
    WpCve("CVE-2023-6875",  "plugin:pods", "<= 3.0.5",
          "SQL injection via crafted REST payload", 9.8),
    WpCve("CVE-2023-32243", "plugin:essential-addons-for-elementor-lite", "<= 5.7.1",
          "Unauth privilege escalation via password reset", 9.8),
    WpCve("CVE-2024-28890", "plugin:forminator", "<= 1.29.3",
          "Reflected XSS in form preview", 6.1),
    WpCve("CVE-2024-27956", "plugin:wp-automatic", "<= 3.92.0",
          "Unauthenticated SQLi via user query", 9.9),
    WpCve("CVE-2024-5932",  "plugin:gutentor", "<= 3.3.7",
          "PHP object injection via AJAX", 9.8),
    WpCve("CVE-2024-4358",  "plugin:elementor-pro", "<= 3.20.2",
          "Unauth arbitrary plugin install", 9.8),
    WpCve("CVE-2023-5360",  "plugin:royal-elementor-addons", "<= 1.3.78",
          "Unauth file upload / RCE", 9.8),
    WpCve("CVE-2023-4634",  "plugin:download-manager", "<= 3.2.82",
          "Unauth RCE via imap_open sink", 9.8),
    WpCve("CVE-2023-5964",  "plugin:backup-migration", "<= 1.3.7",
          "Unauth RCE via PHP filter chain", 9.8),
    WpCve("CVE-2024-1071",  "plugin:ultimate-member", "<= 2.8.2",
          "Unauth SQLi (TGDP)", 9.8),
    WpCve("CVE-2024-3102",  "plugin:woocommerce", "<= 8.7.0",
          "Stored XSS in order-refund page", 6.4),
    WpCve("CVE-2023-40000", "plugin:litespeed-cache", "<= 5.6.5",
          "Unauth stored XSS", 8.3),
    WpCve("CVE-2024-0836",  "plugin:popup-builder", "<= 4.2.3",
          "Unauth stored XSS (SSRF chain)", 7.2),
    WpCve("CVE-2024-4610",  "plugin:paytium", "<= 4.4.3",
          "Missing authz on AJAX actions", 6.5),
]
...
_POPULAR_PLUGINS = [
    ...
    "akismet", "wordfence", "jetpack",
    "litespeed-cache", "really-simple-security", "bricks",
    "forminator", "elementor-pro",
    "essential-addons-for-elementor-lite", "pods",
    "wp-automatic", "ultimate-member", "download-manager",
    "backup-migration", "royal-elementor-addons",
]
...
class WpProbeResult:
    ...
    findings: List[Dict[str, Any]] = None          # list of {kind, location, severity, evidence}

    def __post_init__(self):
        for attr in ("users", "plugins", "themes", "rest_namespaces",
                     "dev_leaks", "findings"):
...
def _probe_core_version(target_url: str, http_get: Callable) -> Optional[str]:
    ...
    try:
        ...
        if s == 200 and body:
            m = re.search(r"""<meta\s+name=['"]generator['"]\s+content=['"][^'"]*?([0-9.]+)""",
                          body, re.IGNORECASE)
            if m:
                return m.group(1)
    except Exception:
...
def _probe_user_enum(target_url: str, http_get: Callable) -> List[Dict[str, Any]]:
    ...
    try:
        s, _h, body = http_get(target_url + "/wp-json/wp/v2/users?per_page=100", 8.0, {})
...
def _probe_plugins(target_url: str, http_get: Callable,
                   slugs: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
def _probe_themes(target_url: str, http_get: Callable,
                  slugs: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
def _probe_dev_leaks(target_url: str, http_get: Callable) -> List[Dict[str, Any]]:
    ...
    paths = [
        "/.env", "/.git/HEAD", "/wp-config.php.bak", "/wp-config.php~",
        "/wp-config-sample.php", "/debug.log", "/wp-content/debug.log",
        "/error_log", "/phpinfo.php", "/.DS_Store", "/readme.html",
    ]
    for p in paths:
        try:
...
def run_wp_probe(target_url: str, http_get: Callable,
                 progress: Optional[Callable] = None,
                 plugin_slugs: Optional[List[str]] = None) -> WpProbeResult:
    ...
    steps: List[Tuple[str, Callable]] = [
        ("core_version", lambda: setattr(r, "core_version", _probe_core_version(target_url, http_get))),
        ("users",        lambda: setattr(r, "users", _probe_user_enum(target_url, http_get))),
        ("xmlrpc",       lambda: _set_xmlrpc(r, target_url, http_get)),
        ("rest_ns",      lambda: setattr(r, "rest_namespaces", _probe_rest_namespaces(target_url, http_get))),
        ("plugins",      lambda: setattr(r, "plugins", _probe_plugins(target_url, http_get, plugin_slugs))),
        ("themes",       lambda: setattr(r, "themes", _probe_themes(target_url, http_get))),
        ("dev_leaks",    lambda: setattr(r, "dev_leaks", _probe_dev_leaks(target_url, http_get))),
    ]
...
def _derive_findings(r: WpProbeResult, target: str) -> List[Dict[str, Any]]:
    ...
    if r.core_version:
        for cve in _CVE_CATALOG:
            if cve.component == "wordpress-core" and cve.active and \
               _version_in_range(r.core_version, cve.affected_range):
                findings.append({
                    "kind": "cve_match",
                    ...
                    "metadata": {"cve": cve.cve_id, "component": cve.component,
    ...
    for p in r.plugins:
        ...
        comp = f"plugin:{slug}"
        for cve in _CVE_CATALOG:
            if cve.component == comp and cve.active and \
               _version_in_range(ver, cve.affected_range):
                findings.append({
                    "kind": "cve_match",
                    ...
                    "metadata": {"cve": cve.cve_id, "component": comp, "version": ver},
                })

    # 3. User enumeration
    if r.users:
        admin_ids = [u for u in r.users if u.get("id") == 1 or
                     (u.get("slug") or "").lower() in ("admin", "administrator")]
        findings.append({
            "kind": "info_leak",
            "location": f"{target}/wp-json/wp/v2/users",
            "severity": "medium" if admin_ids else "low",
            "evidence": (f"User enumeration: {len(r.users)} user(s) disclosed via "
                          f"public REST endpoint. Admin account present: "
                          f"{'yes — id=1 slug=' + (admin_ids[0].get('slug') or '?') if admin_ids else 'not in first 100'}"),
            "metadata": {"count": len(r.users),
    ...
    if r.xmlrpc_open:
        findings.append({
            ...
            "severity": "medium",
            "evidence": (f"xmlrpc.php is reachable (methods hinted: {r.xmlrpc_methods}). "
    ...
    for leak in r.dev_leaks:
        sev = "critical" if leak["path"] in ("/.env", "/wp-config.php.bak",
    ...
    if r.rest_namespaces:
        third_party = [ns for ns in r.rest_namespaces
                       if not (ns.startswith("wp/") or ns.startswith("oembed/") or
                                ns == "wp-site-health/v1" or ns == "wp-block-editor/v1")]
        if third_party:
            findings.append({
                "kind": "rest_authz",
                "location": f"{target}/wp-json/",
                "severity": "medium",
                "evidence": (f"{len(third_party)} third-party REST namespace(s) registered. "
