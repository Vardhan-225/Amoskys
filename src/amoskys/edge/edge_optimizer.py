...
class CompressionEngine:
    ...
    def __init__(self):
        self.algorithms = {
            ...
            "none": {
                "compress": self._no_compression,
                "decompress": self._no_decompression,
            },
        }
        self.stats = {
            "total_compressed": 0,
    ...
    def compress_data(
        self, data: bytes, algorithm: str = "auto"
    ...
    def get_compression_stats(self) -> Dict:
        ...
        avg_time = (
            self.stats["compression_time_ms"] / self.stats["total_compressed"]
...
class EdgeEventBuffer:
    ...
    def add_event(self, event: Dict) -> bool:
        ...
        with self.lock:
            if len(self.buffer) >= self.constraints.max_queue_size:
                ...
                logger.warning(
                    f"Dropped event due to buffer overflow: {dropped.get('event_id', 'unknown')}"
                )
    ...
    def should_flush(self) -> bool:
        ...
        with self.lock:
            ...
            if (
                time_since_flush.total_seconds()
                >= self.constraints.max_batch_age_seconds
            ):
                return True
    ...
    def get_stats(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                "fill_percentage": (len(self.buffer) / self.constraints.max_queue_size)
                * 100,
...
class ResourceMonitor:
    ...
    def _check_constraints(self, metrics: EdgeMetrics):
        ...
        if metrics.cpu_usage_percent > self.constraints.max_cpu_percent:
            violations.append(
                f"CPU usage {metrics.cpu_usage_percent:.1f}% exceeds limit {self.constraints.max_cpu_percent}%"
            )

        if metrics.memory_usage_mb > self.constraints.max_memory_mb:
            violations.append(
                f"Memory usage {metrics.memory_usage_mb:.1f}MB exceeds limit {self.constraints.max_memory_mb}MB"
            )

        if metrics.storage_usage_mb > self.constraints.max_storage_mb:
            violations.append(
                f"Storage usage {metrics.storage_usage_mb:.1f}MB exceeds limit {self.constraints.max_storage_mb}MB"
            )
...
class EdgeOptimizer:
    ...
    def __init__(self, config):
        ...
        if isinstance(config, ResourceConstraints):
            ...
            raise TypeError(
                f"config must be ResourceConstraints or dict, not {type(config)}"
            )
    ...
    def process_telemetry_batch(self, events: List[Dict]) -> Tuple[bytes, Dict]:
        ...
        # Apply compression
        compressed_data, algorithm, ratio = self.compression_engine.compress_data(
            json_data
        )

        # Update statistics
        self.performance_stats["bandwidth_saved_bytes"] += len(json_data) - len(
            compressed_data
        )
    ...
    def should_transmit_batch(self) -> bool:
        ...
        if (
            metrics.memory_usage_mb > self.constraints.max_memory_mb * 0.8
            or metrics.cpu_usage_percent > self.constraints.max_cpu_percent * 0.8
        ):
            return True
    ...
    def get_optimization_stats(self) -> Dict:
        ...
        return {
            "edge_optimization": {
                ...
                "bandwidth_saved_bytes": self.performance_stats[
    ...
    def _log_performance_summary(self, metrics: EdgeMetrics):
        ...
        logger.info(
            f"  Queue: {metrics.queue_depth} events | Compression: {stats['compression']['overall_compression_ratio']:.2f}"
        )
        logger.info(
            f"  Bandwidth saved: {stats['edge_optimization']['bandwidth_saved_bytes']} bytes"
        )
    ...
    def get_resource_usage(self) -> Dict[str, float]:
        ...
        return {
            ...
            "disk_usage_percent": (
                metrics.storage_usage_mb / self.constraints.max_storage_mb
            )
            * 100,
            "memory_percent": (metrics.memory_usage_mb / self.constraints.max_memory_mb)
            * 100,
        }
    ...
    def _compress_data(self, data: bytes, compression_level: int = 6) -> bytes:
        ...
        Returns:
            Compressed data
        """
        compressed, algorithm, ratio = self.compression_engine.compress_data(
            data, algorithm="gzip"
        )
        return compressed

    def _create_batches(
        self, events: List[Any], batch_size: int = 100
    ) -> List[List[Any]]:
        """
    ...
    def _generate_optimization_recommendations(
        self, resource_info: Dict[str, float]
    ) -> List[str]:
        """
        ...
        constraint_cpu = resource_info.get(
            "constraint_cpu_percent", self.constraints.max_cpu_percent
        )
        constraint_memory = resource_info.get(
            "constraint_memory_mb", self.constraints.max_memory_mb
        )

        # CPU recommendations
        if cpu_percent > constraint_cpu:
            recommendations.append(
                f"High CPU usage ({cpu_percent:.1f}% > {constraint_cpu}%): "
                "Reduce collection frequency"
            )
        ...
        if memory_mb > constraint_memory:
            recommendations.append(
                f"High memory usage ({memory_mb:.1f}MB > {constraint_memory}MB): "
        ...
        if cpu_percent > constraint_cpu * 0.8 or memory_mb > constraint_memory * 0.8:
            recommendations.append(
                "Approaching resource limits: Enable aggressive optimization"
            )
...
class EdgeAgentController:
    ...
    def __init__(self, config: Dict):
        ...
        self.constraints = ResourceConstraints(
            ...
            max_concurrent_connections=constraints_config.get(
    ...
    async def _transmit_to_eventbus(self, data: bytes, metadata: Dict):
        ...
        logger.info(
            f"Transmitting {len(data)} bytes to EventBus (compressed from {metadata.get('original_size', 0)} bytes)"
        )
