...
class NetworkFeatureExtractor:
    ...
    def add_packet(self, packet_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if len(flow.packets) > 1:
                iat = packet_data.get('timestamp', time.time()) - flow.packets[-2].get('timestamp', time.time())
    ...
    def _determine_flow_direction(self, packet_data: Dict[str, Any], flow: FlowContext) -> str:
    ...
    def _calculate_features(self, flow: FlowContext) -> NetworkFeatures:
        ...
        forward_bytes = sum(p.get('packet_size', 0) for i, p in enumerate(packets) 
    ...
    def _calculate_threat_score(self, flow: FlowContext) -> float:
        ...
        if 'FIN' in flow.tcp_flags and 'URG' in flow.tcp_flags and 'PSH' in flow.tcp_flags:
            score += 0.7  # Xmas scan
    ...
    def get_baseline_statistics(self) -> Dict[str, Any]:
        ...
        # Top ports
        sorted_ports = sorted(self.baselines['port_usage'].items(), 
