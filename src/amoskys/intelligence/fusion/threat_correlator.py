...
class IntelligenceFusionEngine:
        ...
        return {
            'lateral_movement': {
                ...
                'conditions': [
                    ...
                    {'type': 'network', 'pattern': 'internal_to_internal_new_connection'},
    ...
    def _quick_threat_assessment(self, event: TelemetryEvent) -> float:
        ...
        if 'src_ip' in event.data:
            if event.data['src_ip'] in self.threat_intelligence['iocs']['malicious_ips']:
                risk_score += 0.8
                event.threat_indicators.append('malicious_source_ip')
        
        if 'dst_ip' in event.data:
            if event.data['dst_ip'] in self.threat_intelligence['iocs']['malicious_ips']:
                risk_score += 0.6
    ...
    def _assess_device_risk(self, event: TelemetryEvent) -> float:
        ...
        if event.device_type in [DeviceType.MEDICAL_DEVICE, DeviceType.INDUSTRIAL_CONTROL]:
            risk += 0.1
    ...
    def _evaluate_correlation_rule(self, rule_name: str, rule: Dict[str, Any]) -> Optional[ThreatDetection]:
    ...
    def _find_matching_events(self, events: List[TelemetryEvent], condition: Dict[str, Any]) -> List[TelemetryEvent]:
    ...
    def _calculate_behavioral_anomaly(self, profile: DeviceProfile) -> float:
        ...
        device_events = [e for e in self.telemetry_buffer 
    ...
    def _check_anomaly_indicator(self, events: List[TelemetryEvent], 
                                indicator: str, profile: DeviceProfile) -> bool:
        """Check if specific anomaly indicator is present."""
        if indicator == 'unexpected_protocol':
            if 'communication' in profile.normal_behaviors:
                normal_protocols = profile.normal_behaviors['communication']['protocols']
                for event in events:
            ...
            if 'communication' in profile.normal_behaviors:
                normal_destinations = profile.normal_behaviors['communication']['destinations']
                for event in events:
    ...
    def _hunt_for_apt_indicators(self):
        ...
        for event in self.telemetry_buffer:
            if event.timestamp > cutoff_time and event.event_type == 'network_connection':
    ...
    def _update_trust_score(self, profile: DeviceProfile):
        ...
        recent_anomalies = sum(1 for e in self.telemetry_buffer 
                             if e.device_id == profile.device_id and 
                             e.threat_indicators and 
                             e.timestamp > time.time() - 3600)
        
        trust_score -= recent_anomalies * 0.1
        
        # Reduce trust for threat indicators
        threat_events = sum(1 for e in self.telemetry_buffer 
                          if e.device_id == profile.device_id and 
                          e.risk_score > 0.5)
    ...
    def _update_compliance_status(self, profile: DeviceProfile):
        ...
        if profile.device_type == DeviceType.MEDICAL_DEVICE:
            profile.compliance_status['encryption_required'] = self._check_encryption_compliance(profile)
        
        # Check access control
        profile.compliance_status['access_control'] = self._check_access_control_compliance(profile)
    ...
    def _check_encryption_compliance(self, profile: DeviceProfile) -> bool:
        ...
        recent_events = [e for e in self.telemetry_buffer 
                        if e.device_id == profile.device_id and 
                        e.timestamp > time.time() - 3600]
    ...
    def _handle_threat_detection(self, threat: ThreatDetection):
        ...
        self.logger.warning(f"THREAT DETECTED: {threat.threat_type} - {threat.description}")
    ...
    def _handle_behavioral_anomaly(self, profile: DeviceProfile, anomaly_score: float):
        ...
        self.logger.info(f"Behavioral anomaly detected for device {profile.device_id}: {anomaly_score:.2f}")
    ...
    def _trigger_response_actions(self, threat: ThreatDetection):
        ...
        if threat.threat_level == ThreatLevel.CRITICAL:
            self.logger.critical(f"CRITICAL THREAT: Immediate action required for {threat.threat_type}")
    ...
    def get_threat_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_device_trust': sum(p.trust_score for p in self.device_profiles.values()) / max(len(self.device_profiles), 1)
        }
    ...
    def export_intelligence_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_threats': [threat.__dict__ for threat in self.active_threats.values()],
