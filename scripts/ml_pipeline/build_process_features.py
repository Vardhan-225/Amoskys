...
def build_windowed_features(df, window_size, window_name):
    ...
    for window_start, window_df in df.groupby("window_start"):
        ...
        orphan_ratio = (
            (window_df["ppid"] == 0).sum() / num_processes if num_processes > 0 else 0
        )
        ...
        features.append(
            {
                ...
                "root_proc_ratio": (
                    num_root_procs / num_processes if num_processes > 0 else 0
                ),
                "user_proc_ratio": (
                    num_user_procs / num_processes if num_processes > 0 else 0
                ),
                ...
                "application_ratio": (
                    num_applications / num_processes if num_processes > 0 else 0
                ),
...
def build_features():
    ...
    print(f"📂 Data split:")
    print(
        f"   Train: {len(train_df)} windows ({len(train_df)/len(features_df)*100:.1f}%)"
    )
    ...
    metadata = {
        ...
        "summary_stats": {
            col: {
                ...
                "min": (
                    float(features_df[col].min())
                    if pd.api.types.is_numeric_dtype(features_df[col])
                    else None
                ),
                "max": (
                    float(features_df[col].max())
                    if pd.api.types.is_numeric_dtype(features_df[col])
                    else None
                ),
            }
