...
pkill -f "amoskys.agents.os.macos.process" 2>/dev/null && warn "Killed stale ProcAgent" || true
pkill -f "amoskys.agents.os.macos.auth" 2>/dev/null && warn "Killed stale AuthAgent" || true
pkill -f "amoskys.agents.os.macos.dns" 2>/dev/null && warn "Killed stale DNSAgent" || true
pkill -f "amoskys.agents.os.macos.filesystem" 2>/dev/null && warn "Killed stale FIMAgent" || true
pkill -f "amoskys.agents.os.macos.network" 2>/dev/null && warn "Killed stale FlowAgent" || true
pkill -f "amoskys.agents.os.macos.persistence" 2>/dev/null && warn "Killed stale PersistenceAgent" || true
pkill -f "amoskys.agents.os.macos.peripheral" 2>/dev/null && warn "Killed stale PeripheralAgent" || true
...
python3 -m amoskys.agents.os.macos.process --interval 1 --once --no-heartbeat --log-level WARNING > logs/proc_seed.log 2>&1 || warn "Proc seed had issues (non-fatal)"
python3 -m amoskys.agents.os.macos.auth --interval 1 --once --no-heartbeat --log-level WARNING > logs/auth_seed.log 2>&1 || warn "Auth seed had issues (non-fatal)"
python3 -m amoskys.agents.os.macos.filesystem --interval 1 --once --no-heartbeat --log-level WARNING > logs/fim_seed.log 2>&1 || warn "FIM seed had issues (non-fatal)"
...
python3 -m amoskys.agents.os.macos.process --interval 15 --no-heartbeat --log-level INFO > logs/proc_agent.log 2>&1 &
PROC_PID=$!
PIDS+=($PROC_PID)
ok "Proc Agent (PID $PROC_PID)"

info "Starting Auth Agent (15s interval)..."
python3 -m amoskys.agents.os.macos.auth --interval 15 --no-heartbeat --log-level INFO > logs/auth_agent.log 2>&1 &
AUTH_PID=$!
PIDS+=($AUTH_PID)
ok "Auth Agent (PID $AUTH_PID)"

info "Starting DNS Agent (20s interval)..."
python3 -m amoskys.agents.os.macos.dns --interval 20 --no-heartbeat --log-level INFO > logs/dns_agent.log 2>&1 &
DNS_PID=$!
PIDS+=($DNS_PID)
ok "DNS Agent (PID $DNS_PID)"

info "Starting FIM Agent (20s interval)..."
python3 -m amoskys.agents.os.macos.filesystem --interval 20 --no-heartbeat --log-level INFO > logs/fim_agent.log 2>&1 &
FIM_PID=$!
PIDS+=($FIM_PID)
ok "FIM Agent (PID $FIM_PID)"

info "Starting Flow Agent (20s interval)..."
python3 -m amoskys.agents.os.macos.network --interval 20 --no-heartbeat --log-level INFO > logs/flow_agent.log 2>&1 &
FLOW_PID=$!
PIDS+=($FLOW_PID)
ok "Flow Agent (PID $FLOW_PID)"

info "Starting Persistence Agent (20s interval)..."
python3 -m amoskys.agents.os.macos.persistence --interval 20 --no-heartbeat --log-level INFO > logs/persistence_agent.log 2>&1 &
PERSIST_PID=$!
PIDS+=($PERSIST_PID)
ok "Persistence Agent (PID $PERSIST_PID)"

info "Starting Peripheral Agent (20s interval)..."
python3 -m amoskys.agents.os.macos.peripheral --interval 20 --no-heartbeat --log-level INFO > logs/peripheral_agent.log 2>&1 &
PERIPH_PID=$!
PIDS+=($PERIPH_PID)
...
python3 -m amoskys.agents.os.macos.discovery --interval 60 --no-heartbeat --log-level INFO > logs/device_discovery_agent.log 2>&1 &
DISCO_PID=$!
PIDS+=($DISCO_PID)
ok "Device Discovery Agent (PID $DISCO_PID)"

info "Starting Protocol Collectors Agent (30s interval)..."
python3 -m amoskys.agents.os.macos.protocol_collectors --interval 30 --no-heartbeat --log-level INFO > logs/protocol_collectors_agent.log 2>&1 &
PROTO_PID=$!
PIDS+=($PROTO_PID)
