...
# Stop old processes
pkill -f "amoskys.agents.os.macos.auth.agent" 2>/dev/null || true
pkill -f "amoskys.agents.os.macos.persistence.agent" 2>/dev/null || true
...
PYTHONPATH=src python -m amoskys.agents.os.macos.auth.agent \
    --device-id "$(hostname)" \
...
PYTHONPATH=src python -m amoskys.agents.os.macos.persistence.agent \
    --device-id "$(hostname)" \
