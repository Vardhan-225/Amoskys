    ...
    def test_dashboard_blueprint_registration(self):
        ...
        try:
            ...
            response = self.session.get(
                urljoin(self.base_url, "/dashboard/"), timeout=5
            )
            self.test_result(
                "Dashboard Blueprint Registered", response.status_code in [200, 302]
            )

            # Test cortex dashboard specifically
            response = self.session.get(
                urljoin(self.base_url, "/dashboard/cortex"), timeout=5
            )
    ...
    def test_all_dashboard_pages(self):
        ...
        for dashboard in dashboards:
            try:
                ...
                self.test_result(
                    f"{dashboard.title()} Dashboard Page", response.status_code == 200
                )
                ...
                if response.status_code == 200:
                    ...
                    has_content = any(
                        keyword in content for keyword in expected_content[dashboard]
                    )
                    self.test_result(
                        f"{dashboard.title()} Dashboard Content", has_content
                    )
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        self.test_result(
                            f"API JSON Response {endpoint}", isinstance(data, dict)
                        )
                    except json.JSONDecodeError:
                        self.test_result(
                            f"API JSON Response {endpoint}",
                            False,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for link in expected_links:
                    ...
                    self.test_result(
                        f"Navigation Link {dashboard_name.title()}", has_link
                    )
        ...
        try:
            ...
            try:
                ...
                if event[0] == "initial_data":
                    ...
                    self.test_result(
                        "SocketIO Initial Data",
                        False,
                        f"Expected initial_data, got {event[0]}",
                    )
            except Exception:
                self.test_result(
                    "SocketIO Initial Data", False, "Timeout waiting for data"
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    has_base_elements = all(
                        element in content
                        for element in ["AMOSKYS", "Chart.js", "socket.io"]
                    )

                    self.test_result(
                        f"{dashboard.title()} Template Rendering",
                ...
                self.test_result(
                    f"{dashboard.title()} Template Rendering", False, str(e)
                )
    ...
    def test_performance_metrics(self):
        ...
        for dashboard in dashboards:
            try:
                ...
                self.test_result(
                    f"{dashboard.title()} Load Performance", load_time < 3.0
                )

                if load_time >= 3.0:
                    self.log(
                        f"⚠️  {dashboard.title()} loaded in {load_time:.2f}s", "WARN"
                    )
