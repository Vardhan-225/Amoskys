...
class ComponentTester:
            ...
            if data:
                ...
                if top_cpu:
                    ...
                    for i, proc in enumerate(top_cpu[:3], 1):
                        logger.info(
                            f"    {i}. {proc['name']} - {proc['cpu_percent']:.1f}%"
                        )
    ...
    async def test_score_junction(self):
        ...
        try:
            ...
            logger.info(
                f"  Created ScoreJunction with {junction.correlation_window}s window"
            )
            logger.info(
                f"  Loaded {len(junction.correlation_engine.rules)} correlation rules"
            )
            logger.info(
                f"  Correlation rules: {[r['name'] for r in junction.correlation_engine.rules]}"
            )

            # Get statistics
            stats = junction.get_statistics()
            logger.info(
                f"  Statistics: {stats['events_processed']} events, {stats.get('correlations_found', 0)} correlations"
            )
    ...
    async def test_wal_database(self):
        ...
        try:
            ...
            # Check table exists
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='wal'"
            )
...
async def main():
    ...
    parser.add_argument(
        "--correlation", action="store_true", help="Test ScoreJunction only"
    )
    parser.add_argument(
        "--eventbus", action="store_true", help="Test EventBus connection only"
    )
    ...
    if args.all or not any(
        [args.snmp, args.proc, args.correlation, args.eventbus, args.wal]
    ):
