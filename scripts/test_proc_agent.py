...
def main():
    ...
    # Top CPU consumers
    top_cpu = sorted(processes, key=lambda p: p.get("cpu_percent", 0), reverse=True)[
        :10
    ]
    print("\n🔥 Top 10 CPU consumers:")
    for i, proc in enumerate(top_cpu, 1):
        print(
            f"  {i}. {proc['name']:<20} PID {proc['pid']:<8} CPU {proc.get('cpu_percent', 0):.1f}%"
        )

    # Top memory consumers
    top_mem = sorted(processes, key=lambda p: p.get("memory_percent", 0), reverse=True)[
        :10
    ]
    print("\n💾 Top 10 Memory consumers:")
    for i, proc in enumerate(top_mem, 1):
        print(
            f"  {i}. {proc['name']:<20} PID {proc['pid']:<8} MEM {proc.get('memory_percent', 0):.1f}%"
        )
