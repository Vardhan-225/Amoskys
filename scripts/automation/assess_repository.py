...
class RepositoryAssessor:
    ...
    def analyze_dependencies(self) -> AssessmentResult:
        ...
        security_configs = [".safety", "safety-policy.json", ".bandit", "bandit.yaml"]

        has_security = any(
            (self.project_root / sfile).exists() for sfile in security_configs
        )
        if has_security:
                ...
                try:
                    with open(config_path, "r") as f:
                        ...
                        if "coverage" in content.lower():
                            details.append(
                                f"✓ Coverage configuration found in {config_file}"
                            )
            ...
            if venv_python.exists():
                ...
                if result.returncode == 0:
                    ...
                    if collected_tests > 0:
                        details.append(
                            f"✓ {collected_tests} tests collected successfully"
                        )
                        score += 25
                    else:
                ...
                details.append(
                    "⚠ Virtual environment not found - cannot run test analysis"
                )
    ...
    def analyze_code_quality(self) -> AssessmentResult:
        ...
        if py_files:
            ...
            metrics["docstring_percentage"] = (
                (files_with_docstrings / len(sample_files)) * 100 if sample_files else 0
            )

            # Quality scoring
            if metrics["docstring_percentage"] > 70:
                details.append(
                    f"✓ Good documentation coverage ({metrics['docstring_percentage']:.1f}%)"
                )
                score += 20
            elif metrics["docstring_percentage"] > 40:
                details.append(
                    f"⚠ Moderate documentation coverage ({metrics['docstring_percentage']:.1f}%)"
                )
                score += 10
                recommendations.append("Improve docstring coverage")
            else:
                details.append(
                    f"✗ Low documentation coverage ({metrics['docstring_percentage']:.1f}%)"
                )
            ...
            avg_functions_per_file = (
                total_functions / len(sample_files) if sample_files else 0
            )
            avg_classes_per_file = (
                total_classes / len(sample_files) if sample_files else 0
            )
        ...
        type_hint_percentage = (
            (type_hint_files / len(sample_files)) * 100 if sample_files else 0
        )
        ...
        self.print_status(
            f"Python files analyzed: {len(sample_files) if py_files else 0}"
        )
        ...
        self.print_status(
            f"Documentation coverage: {metrics.get('docstring_percentage', 0):.1f}%"
        )
    ...
    def analyze_security_posture(self) -> AssessmentResult:
        ...
        security_deps = ["cryptography", "pyjwt", "bcrypt", "passlib"]
        ...
        if len(found_security_deps) >= 2:
            details.append(
                f"✓ Good security dependencies: {', '.join(found_security_deps)}"
            )
            score += 15
        elif len(found_security_deps) >= 1:
            details.append(
                f"⚠ Basic security dependencies: {', '.join(found_security_deps)}"
            )
            score += 8
        else:
    ...
    def analyze_documentation(self) -> AssessmentResult:
        ...
        if docs_dir.exists():
            ...
            if len(doc_files_in_dir) >= 10:
                details.append(
                    f"✓ Extensive documentation ({len(doc_files_in_dir)} files)"
                )
                score += 15
            elif len(doc_files_in_dir) >= 5:
                details.append(
                    f"⚠ Moderate documentation ({len(doc_files_in_dir)} files)"
                )
                score += 10
            else:
                details.append(
                    f"✗ Limited documentation ({len(doc_files_in_dir)} files)"
                )
        ...
        if py_files:
            ...
            if docstring_percentage >= 70:
                details.append(
                    f"✓ Good inline documentation ({docstring_percentage:.1f}%)"
                )
                score += 10
            else:
        ...
        self.print_status(
            f"Docs directory files: {metrics.get('docs_directory_files', 0)}"
        )
    ...
    def generate_overall_assessment(self) -> Dict[str, Any]:
        ...
        if critical_issues:
            priority_recommendations.append(
                f"Address critical issues in: {', '.join(critical_issues)}"
            )
    ...
    def print_summary_report(self) -> None:
        ...
        for result in sorted(self.results, key=lambda x: x.score, reverse=True):
            ...
            print(
                f"  {status_symbol[result.status]} {result.component:<25} {result.score:>3}/100"
            )
        ...
        if overall["overall_score"] >= 85:
            ...
            print(
                "  ⚠️  Needs attention. Prioritize critical issues and core improvements"
            )
        else:
            print(
                "  🚨 Critical state. Immediate action required on foundational issues"
            )
    ...
    def run_full_assessment(self) -> Dict[str, Any]:
        ...
        for assessment_func in assessments:
            try:
                ...
                self.print_status(
                    f"Assessment error in {assessment_func.__name__}: {e}", "CRITICAL"
                )
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, help="Output file for detailed JSON report"
    )
