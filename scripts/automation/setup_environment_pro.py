...
class EnvironmentManager:
    ...
    def check_python_version(self) -> bool:
        ...
        if sys.version_info >= (3, 13):
            self.print_status(
                f"Python {self.python_version} - Latest version detected ✓", "SUCCESS"
            )
        else:
            self.print_status(
                f"Python {self.python_version} - Compatible version ✓", "SUCCESS"
            )
    ...
    def select_requirements_file(self, mode: str) -> Path:
        ...
        req_file = self.project_root / requirements_map.get(
            mode, "requirements/requirements.txt"
        )
    ...
    def install_requirements(self, mode: str = "development") -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=900  # 15 minutes max
            )
    ...
    def verify_installation(self) -> bool:
        ...
        for module, description in test_imports:
            try:
                ...
                if result.returncode == 0:
                    self.print_status(
                        f"{description}: {result.stdout.strip()}", "SUCCESS"
                    )
                else:
    ...
    def run_basic_tests(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.print_status(
                    "Some tests failed - check full test suite", "WARNING"
                )
    ...
    def print_summary(self, success: bool) -> None:
        ...
        if success:
            self.print_status(
                "🎉 AMOSKYS environment setup completed successfully!", "SUCCESS"
            )
            ...
            if platform.system() == "Windows":
                self.print_status(
                    f"   source {self.venv_path}/Scripts/activate", "INFO"
                )
            else:
            ...
            self.print_status(
                "Please check the error messages above and try again", "ERROR"
            )
...
Examples:
  ...
  python setup_environment_pro.py --minimal          # Minimal dependencies only
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force clean rebuild of environment"
    )
