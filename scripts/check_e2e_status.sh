...
AUTH_RUNNING=$(pgrep -f "amoskys.agents.os.macos.auth.agent" | wc -l)
PERSIST_RUNNING=$(pgrep -f "amoskys.agents.os.macos.persistence.agent" | wc -l)
...
if [ "$AUTH_RUNNING" -gt 0 ]; then
    echo -e "  ${GREEN}✓${NC} AuthGuardAgent ($(pgrep -f amoskys.agents.os.macos.auth.agent))"
else
    echo -e "  ${RED}✗${NC} AuthGuardAgent (not running)"
fi

if [ "$PERSIST_RUNNING" -gt 0 ]; then
    echo -e "  ${GREEN}✓${NC} PersistenceGuardAgent ($(pgrep -f amoskys.agents.os.macos.persistence.agent))"
else
