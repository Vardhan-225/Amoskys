...
INADS_CLUSTERS = {
    "process_tree": {
        "fields": ["pid", "ppid", "process_name", "exe", "cmdline"],
        "obs_domains": ["process"],
        "min_samples": 1000,
    },
    "network_lstm": {
        "fields": ["remote_ip", "local_port", "protocol", "bytes_out"],
        "obs_domains": ["flow"],
        "min_samples": 500,
    },
    "kill_chain": {
        "fields": ["kill_chain_stage", "mitre_techniques"],
        "obs_domains": [],
        "min_samples": 50,
    },
    "system_anomaly": {
        "fields": ["username", "detection_source", "collection_agent"],
        "obs_domains": ["auth", "realtime_sensor"],
        "min_samples": 500,
    },
    "file_path": {
        "fields": ["path", "file_name", "sha256"],
        "obs_domains": ["filesystem"],
        "min_samples": 500,
    },
}
...
class V:
    ...
    def show(self):
        icon = (
            ...
            else "\u26a0\ufe0f" if self.passed else "\u274c"
        )
        status = (
            "PASS"
            if self.passed and not self.warnings
            else "WARN" if self.passed else "FAIL"
        )
...
def v1_mandate(conn):
    ...
    for f in MANDATORY_FIELDS:
        if f in c:
            n = sc(
                conn,
                f"SELECT COUNT(*) FROM security_events WHERE {f} IS NOT NULL AND {f} != ''",
            )
        else:
            n = sc(
                conn,
                f"SELECT COUNT(*) FROM security_events WHERE json_extract(raw_attributes_json, '$.{f}') IS NOT NULL",
            )
        pct = n / total * 100
...
def v2_observations(conn):
    ...
    has_raw = sc(
        conn,
        "SELECT COUNT(*) FROM observation_events WHERE raw_attributes_json IS NOT NULL AND raw_attributes_json != '{}' AND raw_attributes_json != ''",
    )
    ...
    try:
        rows = conn.execute(
            "SELECT domain, COUNT(*) FROM observation_events GROUP BY domain ORDER BY COUNT(*) DESC"
        ).fetchall()
    ...
    for field, keys in [
        ...
        ("exe", ["exe"]),
        ("remote_ip", ["remote_ip", "dst_ip", "src_ip"]),
        ("username", ["username", "conn_user"]),
    ]:
        conds = " OR ".join(
            f"json_extract(raw_attributes_json, '$.{k}') IS NOT NULL" for k in keys
        )
        n = sc(
            conn,
            f"SELECT COUNT(*) FROM observation_events WHERE raw_attributes_json != '{{}}' AND ({conds})",
        )
...
def v3_soma(db_path):
    ...
    v.d(
        "isolation_forest",
        f"{iso.get('status')} samples={iso.get('samples',0):,} anomaly_rate={iso.get('anomaly_rate',0):.3f}",
    )
    ...
    if fp.exists():
        try:
            ...
            if len(present) < 8:
                v.fail(
                    f"Missing mandate features: {[f for f in mandate if f not in fc]}"
                )
        except ImportError:
...
def v4_agents(conn):
    ...
    rows = conn.execute(
        "SELECT collection_agent, COUNT(*) FROM security_events GROUP BY collection_agent ORDER BY COUNT(*) DESC"
    ).fetchall()
...
def v5_quality(conn):
    ...
    if total == 0:
        ...
        return v
    app = sc(
        conn, "SELECT COUNT(*) FROM security_events WHERE event_category='app_launch'"
    )
    fe80 = sc(
        conn, "SELECT COUNT(*) FROM security_events WHERE remote_ip LIKE '%fe80%'"
    )
    self_d = sc(
        conn,
        "SELECT COUNT(*) FROM security_events WHERE cmdline LIKE '%collect_and_store%' OR cmdline LIKE '%analyzer_main%'",
    )
    null_desc = sc(
        conn,
    ...
    if app > 0:
        v.fail(f"{app} app_launch events in security_events")
    if fe80 > 0:
        v.fail(f"{fe80} link-local IPs")
    if null_desc > 0:
        v.warn(f"{null_desc} null descriptions")
    try:
        rows = conn.execute(
            "SELECT final_classification, COUNT(*) FROM security_events GROUP BY final_classification"
        ).fetchall()
...
def v6_scoring(db_path):
    ...
    try:
        ...
        for i, c in enumerate(fc):
            if c == "hour_of_day":
                normal[0, i], anomalous[0, i] = 14.0, 3.0
            elif c == "is_business_hours":
                normal[0, i] = 1.0
            elif c == "has_process_context":
                normal[0, i], anomalous[0, i] = 1.0, 1.0
            elif c == "has_executable":
                normal[0, i], anomalous[0, i] = 1.0, 1.0
            elif c == "is_system_exe":
                normal[0, i] = 1.0
            elif c == "has_network_context":
                anomalous[0, i] = 1.0
            elif c == "is_root_user":
                anomalous[0, i] = 1.0
            elif c == "has_suspicious_tokens":
                anomalous[0, i] = 1.0
            elif c == "requires_investigation":
                anomalous[0, i] = 1.0
...
def v7_inads(conn):
    ...
    v.d(
        "training_pool",
        f"{total_sec + total_obs:,} (sec={total_sec:,} + obs={total_obs:,})",
    )
    all_ready = True
    c = cols(conn, "security_events")
    for name, spec in INADS_CLUSTERS.items():
        ...
        if spec["obs_domains"]:
            ...
            obs_n = sc(
                conn,
                f"SELECT COUNT(*) FROM observation_events WHERE domain IN ({dl}) AND raw_attributes_json != '{{}}' AND raw_attributes_json IS NOT NULL",
            )
        tot = sec_n + obs_n
        ...
        v.d(
            f"cluster_{name}",
            f"{status}: {tot:,} (sec={sec_n:,}+obs={obs_n:,}, need={spec['min_samples']:,})",
        )
