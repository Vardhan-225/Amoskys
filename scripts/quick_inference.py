...
def run_inference(models, n_samples=10):
    ...
    for i in range(n_samples):
        ...
        for name, model in models.items():
            ...
            score = (
                model.score_samples(df)[0] if hasattr(model, "score_samples") else None
            )
        ...
        if is_anomaly:
            ...
            print(
                f"           Top metrics: {', '.join([f'{k}={v:.0f}' for k, v in metrics])}"
            )
...
def main():
    ...
    print(
        f"   Anomalies detected: {anomaly_count} ({anomaly_count/len(results)*100:.1f}%)"
    )
