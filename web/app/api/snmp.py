    ...
    try:
        ...
        for row in cursor.fetchall():
            devices.append({
                ...
                'last_seen': datetime.fromtimestamp(row['last_seen'] / 1e9).isoformat(),
                'event_count': row['event_count'],
                'status': 'online' if (datetime.now().timestamp() * 1e9 - row['last_seen']) < 120e9 else 'offline'
            })
    ...
    try:
        ...
        for row in cursor.fetchall():
            metrics.append({
                ...
                'collection_time': datetime.fromtimestamp(row['start_time'] / 1e9).isoformat() if row['start_time'] else None
            })
