        ...
        for row_id, idem, ts_ns, env_bytes in rows:
            try:
                ...
                if envelope.HasField('device_telemetry'):
                    ...
                    event_data['device_type'] = universal_telemetry_pb2.DeviceType.Name(dt.device_type)

                    # Extract metrics from events
                    metrics = []
                    for event in dt.events:
                        if event.event_type == universal_telemetry_pb2.TelemetryEvent.METRIC:
                            metric_data = {
                                'name': event.metric.name,
                                'type': universal_telemetry_pb2.MetricType.Name(event.metric.metric_type),
...
def get_agent_summary():
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT bytes FROM wal ORDER BY ts_ns DESC LIMIT 100"
        )
        rows = cursor.fetchall()
        conn.close()
            ...
            try:
                ...
                if envelope.HasField('device_telemetry'):
                    ...
                    if device_id not in agents:
                        agents[device_id] = {
                            'device_id': device_id,
                            'device_type': universal_telemetry_pb2.DeviceType.Name(dt.device_type),
                            'event_count': 0,
                            'latest_metrics': {},
                            'last_seen': None
                        }
                    ...
                    if agents[device_id]['last_seen'] is None or ts > agents[device_id]['last_seen']:
...
def get_telemetry_stats():
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*), MIN(ts_ns), MAX(ts_ns) FROM wal"
        )
        ...
        if min_ts and max_ts:
            stats['earliest_event'] = datetime.fromtimestamp(min_ts / 1e9, tz=timezone.utc).isoformat()
            stats['latest_event'] = datetime.fromtimestamp(max_ts / 1e9, tz=timezone.utc).isoformat()
