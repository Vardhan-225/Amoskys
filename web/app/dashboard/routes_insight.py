...
def api_insight():
    ...
    if insight_service is None:
        return (
            jsonify({"error": "unavailable", "message": "insight service not loaded"}),
            200,
        )
    ...
    return (
        jsonify(
            insight_service.get_model(
                force=force, allowed_device_ids=allowed, cache_key=cache_key
            )
        ),
        200,
    )
...
def api_device():
    ...
    return (
        jsonify(
            insight_service.get_device_model(
                device_id=device_id,
                force=force,
