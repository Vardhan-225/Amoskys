...
def find_process_by_pattern(pattern: str) -> Optional[psutil.Process]:
    ...
    try:
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                if pattern.lower() in str(proc.cmdline()).lower() or pattern.lower() in proc.name().lower():
                    return proc
    ...
    if proc:
        try:
            ...
            return {
                ...
                'port_open': is_port_open(agent_config['port']) if agent_config['port'] else None,
...
def health_check_agent(agent_id: str) -> Dict[str, Any]:
    ...
    if status['cpu_percent'] > 90:
        health_result['warnings'].append(f'High CPU usage: {status["cpu_percent"]:.1f}%')
    
    if status['memory_mb'] > 500:  # 500MB threshold
        health_result['warnings'].append(f'High memory usage: {status["memory_mb"]:.1f}MB')
...
def _build_startup_command(agent_id: str, config: Dict[str, Any]) -> Optional[List[str]]:
