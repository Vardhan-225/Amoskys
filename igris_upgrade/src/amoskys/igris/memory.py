...
class IGRISMemory:
    ...
    def load_state(self) -> dict:
        ...
        rows = self._conn.execute(
            "SELECT key, value FROM tactical_state"
        ).fetchall()
        state = {}
        for row in rows:
            try:
    ...
    def record_directive_outcome(
        self, directive_id: int, outcome: str, useful: int
    ):
    ...
    def get_directive_stats(self) -> dict:
        ...
        total = self._conn.execute(
            "SELECT COUNT(*) FROM directive_history"
        ).fetchone()[0]
    ...
    def complete_investigation(
        self, request_id: int, result: dict, verdict: str
    ):
    ...
    def soma_get_baseline(
        self, event_category: str, limit: int = 20
    ) -> List[dict]:
